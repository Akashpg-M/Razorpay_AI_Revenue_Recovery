...
`/observability` reads durable queue, execution, case, and promise truth. `/health/live` proves the process is running; `/health/ready` requires PostgreSQL, the decision service where applicable, and schema `phase_55`. Redis is reported as optional. Development alerts flag queue lag, execution failures, and overdue promises.
